...
from text_extraction.apis.clients.opensource import OpenSourceClient
...
def test__download_in_parallel_success(publication_list, caplog):
    ...
    # Patch run_publisher
    with patch("text_extraction.apis.router.ApiRouter._run_publisher") as mock_run:
...
def test__download_in_parallel_api_error(mock_publisher_failure, mock_publisher_working, caplog):
    ...
    with patch("text_extraction.apis.router.ApiRouter._run_publisher", side_effect=fake_run) as mock_run:
        results = ApiRouter._download_in_parallel(client_list, max_threads=4)
...
def test_throw_at_opensource_success(publication_list):
    ...
    with patch('text_extraction.apis.router.OpenSourceClient') as MockOpenSourceClient:
...
def test_throw_at_opensource_when_flag_false(publication_list):
    ...
    with patch("text_extraction.apis.router.OpenSourceClient") as MockOS:
        router.throw_at_opensource()
