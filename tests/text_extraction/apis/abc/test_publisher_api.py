...
from config import DB_CACHE_FILE
...
@patch("text_extraction.apis.abc.publisher_api.API_URL_TO_NAME", {"unpaywall": "https://api.unpaywall.org"})
@patch("text_extraction.apis.abc.publisher_api.API_KEY_TO_NAME", {"unpaywall": "fake-key"})
@patch("text_extraction.apis.abc.publisher_api.USER_EMAIL", "test@example.com")
@patch("text_extraction.apis.abc.publisher_api.DOWNLOAD_DIR", "/tmp/downloads")
...
def test__test_url_success():
    ...
    with patch("text_extraction.apis.abc.publisher_api.requests.get", return_value=mock_response) as mock_get:
...
def test__test_url_failure(publications):
    api = PublicationApiDummy(name="unpaywall", publication_list=publications)
    with patch("text_extraction.apis.abc.publisher_api.requests.get")as mock_get:
        mock_get.side_effect = requests.exceptions.RequestException("Network error")
        result = api._test_url()
        assert result is False

@patch("text_extraction.apis.abc.publisher_api.DOWNLOAD_DIR", Path("/tmp/downloads"))
def test__build_download_filepath():
    api = PublicationApiDummy(name="unpaywall", publication_list=[])
    with patch("text_extraction.apis.abc.publisher_api.random.choices", return_value=list("1234567890")):
...
def test_download_paper_from_url_http_error():
    ...
    with patch("text_extraction.apis.abc.publisher_api.requests.get", return_value=mock_response):
...
def test__download_pdf_if_valid_request_exception():
    ...
    with patch("text_extraction.apis.abc.publisher_api.requests.get",
...
def test__cache_successful_download(publications):
    ...
    # Success.
    with patch("text_extraction.apis.abc.publisher_api.insert_row", return_value=None) as mock_insert:
