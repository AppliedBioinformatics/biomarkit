...
from text_extraction.visualisation.scopus_query_report import (
    _plt_raw_publisher_frequency,
    _plt_publisher_group_frequency,
    _plt_cumulative_frequency_of_publishers,
)
from text_extraction.visualisation.download_report import (
...
def build_text_extraction_report(
    ...
    out_file: Path = None,
    scopus_report: bool = False,
    timings: Optional[Dict[str, float]] = None,
):
    """
    Builds a single combined HTML report for the text-extraction pipeline.

    Section 1 — Scopus Input Summary (3 charts, only when scopus_report=True):
    ...
    out_file : Path, optional
        Output filename. Defaults to text_extraction_report_<timestamp>.html.
    scopus_report : bool, default False
        If True, prepend the Scopus input summary charts to the report.
    ...
    # --- Section 1: Scopus input summary (opt-in) ---
    scopus_figs = []
    if scopus_report:
        scopus_figs = [
    ...
    if scopus_report:
        ...
        subtitle = (
            "This report summarises the full-text download results: "
            "download success rates, cache status, and year distribution."
        )

    if out_file is None:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
        out_file = f"text_extraction_report_{timestamp}.html"
